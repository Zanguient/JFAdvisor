...
#property indicator_color2  clrBlack,clrRoyalBlue
...
//--- indicator buffers mapping
   ...
   handle_ma2 = iMA(_Symbol, _Period, ma2, 0, mode, price);
   handle_ma3 = iMA(_Symbol, _Period, ma3, 0, mode, price);
   handle_ma4 = iMA(_Symbol, _Period, ma4, 0, mode, price);
...
//+------------------------------------------------------------------+
   ...
   CopyBuffer(handle_ma4, 0, 0, qtdCopiada, array_ma4);

   for(int i = inicio; i < rates_total; i++) {
      media[i] = ( array_ma1[i] + array_ma2[i] + array_ma3[i] + array_ma4[i]) / 4;

      mediamovel[i] = (media[i] + media[i - 1] + media[i - 2] + media[i - 3]) / 4;
